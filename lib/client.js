window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** Chain entry body: renders the ideal card at the dispatch turn (inline in the message flow). */
    ...
    function apply(ctx) {
      ...
      // Inline in the conversation: the ideal swarm card mounts at the turn
      // where the swarm was dispatched (middle of the message flow, like a chat
      // or think block) — not a bottom dock. This is a chain seat: select
      // narrows on the turn boundary, and the component declines via the
      // synchronous conversation snapshot.
