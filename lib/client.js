window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var DICT_ZH = {
      ...
      "phase_executing": "执行中",
      "phase_complete": "完成",
      "hint": "这是 dsh-agent-swarm-ui 所渲染的原始 swarm 状态的原始数据可视化。用它来核对数据是否完整——这里缺失的数据，在富界面中同样缺失。"
    };
    var DICT_EN = {
      "tab": "Swarm",
      "tabIdeal": "Ideal UI",
      ...
      "hint": "A primitive data visualization of the raw swarm state that dsh-agent-swarm-ui renders. Use it to confirm the data is complete — anything missing here is also missing in the rich UI."
    };
    ...
    var STYLE = [
      ...
      ".das-card{background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l1);border-radius:10px;margin-bottom:14px;padding:14px 16px;max-height:360px;overflow-y:auto}",
      ".das-card-roster{max-height:none;overflow:visible}",
      ...
      ".das-row-open{border-color:var(--dsw-alias-state-business-primary)}",
      ...
      ".das-agent{display:flex;flex-direction:column}",
      ".das-chevron{flex:none;color:var(--dsw-alias-label-secondary);font-size:11px;width:14px;line-height:1}",
      ".das-agent-detail{max-height:240px;overflow-y:auto;margin-top:6px;border:1px solid var(--dsw-alias-border-l1);border-radius:8px;padding:8px 10px;background:var(--dsw-alias-bg-layer-2)}",
    ...
    function SwarmView(props) {
      ...
      return h("div", { className: "das-root" },
        ...
        agents.length > 0 ? h("div", { className: "das-card das-card-roster" },
          ...
          h("div", { className: "das-list" }, agents.map(function (a) {
            ...
            var modelTag = (typeof a.model === "string" && a.model !== "")
              ...
              : null;
            var detail = agentDetail(a);
            var isOpen = !!expanded[a.id];
            return h("div", { className: "das-agent", key: a.id },
              h("div", {
                className: "das-row" + (isOpen ? " das-row-open" : ""),
                onClick: function () { toggleAgent(a.id); },
                style: { cursor: "pointer" }
              },
                h("span", { className: "das-chevron" }, isOpen ? "▾" : "▸"),
              ...
              isOpen && detail ? h("div", { className: "das-agent-detail" }, detail) : null
            );
          }))
        ) : null,
        ...
        (Array.isArray(swarm.resources) && swarm.resources.length > 0) ? h("div", { className: "das-card" },
          h("div", { className: "das-title" }, t("resources")),
        ...
        (Array.isArray(swarm.lines) && swarm.lines.length > 0) ? h("div", { className: "das-card" },
    ...
    function NativeSwarmView(props) {
      ...
      // No min-height: the card is content-driven, so collapsing the panel
      // leaves no blank space below the header.
