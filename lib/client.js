window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function NativeSwarmView(props) {
      ...
      // A centered card (720px), not a full-width strip.
      var style = { width: "100%", maxWidth: "720px", margin: "0 auto", overflow: "auto", background: "#0c0d0f", borderRadius: "12px" };
      if (props.inline) {
    ...
    /**
     * Live swarm card (input.dock seat, above the composer): as soon as the
     * main agent dispatches the swarm (the first `swarm` tool call with
     * recruit/plan/confirm appears in the conversation snapshot), render the
     * ideal card. The card live-updates via its own /swarm/state poll, so the
     * agent cards are visible WHILE subagents run — from recruiting onward.
     */
    function LiveSwarmCard(props) {
      var useSession = props.useSession;
      var dispatched = typeof useSession === "function"
        ? useSession(function (s) { return firstSwarmTurn(s) >= 0; })
        : false;
    ...
    function apply(ctx) {
      ...
      // Live in the chat: the ideal swarm card mounts above the composer as
      // soon as the swarm is dispatched (so it is visible from recruiting
      // onward, not only after the turn closes). The dock renders entries as a
      // centered card column; the panel is a 720px card, not a full-width strip.
