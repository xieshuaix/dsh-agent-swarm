window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** The ideal swarm UI, embedded as an iframe served by this plugin's host half. */
    function idealFrameSrc(props) {
      return UI_ROUTE + "/?session=" + encodeURIComponent(props.sessionId || "");
    }
    ...
    function apply(ctx) {
      ...
      // Second tab: the ideal swarm UI (dsh-agent-swarm-ui) embedded natively.
      ctx.slots.inject("conversation.view", function () {
        return ctx.slots.register({
          name: "conversation.view",
          id: "ideal",
          order: 36,
          locale: NS,
          label: function () { return t("tabIdeal"); }
        }, function (props) {
          return h("iframe", {
            src: idealFrameSrc(props),
            title: "Ideal swarm UI",
