window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** Chain entry body: renders the ideal card only at the dispatch turn. */
    ...
    function apply(ctx) {
      ...
      // Inline in the chat: the ideal swarm agent card mounts at the message
      // where the swarm was dispatched. This is a chain seat — select narrows on
      // the turn boundary (it can't see the async swarm state), and the component
      // declines via the synchronous conversation snapshot.
