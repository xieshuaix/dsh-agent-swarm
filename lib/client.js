window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Live swarm dock (mounted above the composer, not at the turn tail): as
     * soon as the main agent dispatches the swarm (the first `swarm` tool call
     * with recruit/plan/confirm appears in the conversation snapshot), render
     * the ideal panel. It live-updates via its own /swarm/state poll, so the
     * agent cards are visible WHILE subagents run — not only after the turn
     * (a toy swarm is one long turn, so a turn-tail mount always looked "done").
     */
    function LiveSwarmDock(props) {
    ...
    function apply(ctx) {
      ...
      // Live swarm cards: mount the ideal panel above the composer (the
      // input.dock seat) as soon as the swarm is dispatched, so the agent cards
      // are visible while subagents run — not just after the turn closes.
