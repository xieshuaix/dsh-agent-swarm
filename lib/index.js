...
function apply(ctx, config = {}) {
  ...
  function listArtifacts(childId, root) {
    ...
    if (meta !== undefined) {
      ...
      fileCandidates = [...topLevelFiles(meta.root)].filter((fileName) => !(meta.beforeFiles ?? new Set()).has(fileName));
    ...
    // Top-level directories (ownership via marker → event-reference).
    const dirs = dirCandidates.filter((dirName) => childOwnsDir(childId, dirName, base));
    for (const dirName of dirs) {
      for (const file of listFiles(join(base, dirName))) {
        pushFile(join(base, dirName, file.rel), `${dirName}/${file.rel}`, file.size);
      }
    }

    // Root-level files the child wrote directly (no folder). Attribute via the
    // child's session-event reference to the file name.
    for (const fileName of fileCandidates) {
      if (fileName.startsWith(".")) continue;
      if (!childReferencedDir(childId, fileName)) continue;
      let size = "";
  ...
  ctx.inject(["commands"], (scope) => {
    scope.commands.register({
      ...
      input: { hint: "recruit <name> [role] [task] [--model=X] [--effort=Y] [--outline] [--prompt=ROLE] | plan <title>[@owner] [; ...] [--objective=...] | confirm | cancel | summarize <text>" },
  ...
  ctx.tools.register({
      ...
      description:
        ...
        "Use action=recruit to add subagents (each with name, role, task, and optional model/reasoningEffort/outlinePlan/rolePrompt), " +
        "action=plan to record the orchestration plan (items with an optional ownerId), " +
        "action=confirm to start execution, and action=summarize to record the end-of-swarm summary. " +
