...
function apply(ctx, config = {}) {
  ...
  const recruit = (session, agents, concurrency) => {
    ...
    // Re-open the lifecycle for a new batch: a fresh swarm starts at
    // "recruiting", and a completed swarm re-enters "planning" so its newly
    // recruited agents can be planned + confirmed — otherwise they'd stay
    // "queued" under a "complete" phase and never run.
    if (list.length > 0 && (swarm.phase === "recruiting" || swarm.phase === "complete")) {
