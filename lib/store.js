...
export function emptySwarm() {
  return {
    ...
    permissions: {},
    messages: [],
    // Main-agent resume: when `resumeMainWhenDone` is true (opt-in at confirm), a swarm
    // that reaches `complete` wakes the idle main agent to review the subagents'
    // work. `resumeSent` marks the one-shot resume as delivered.
    resumeMainWhenDone: false,
    resumeSent: false,
    updatedAt: 0
  };
}
...
export function coerceSwarm(raw) {
  ...
  return {
    ...
    permissions,
    messages,
    resumeMainWhenDone: raw.resumeMainWhenDone === true,
    resumeSent: raw.resumeSent === true,
