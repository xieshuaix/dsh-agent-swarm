...
| 19 | The ideal card spanned the whole chat width as a bottom panel (wrong design), and the Swarm tab was replaced by the ideal UI (it should stay primitive) | I mounted the ideal panel at `conversation.input.dock` (a full-width band above the composer) and pointed `conversation.view` at `NativeSwarmView` | Restore `conversation.view` as the **primitive** thin `SwarmView` (basic phase/roster/plan/summary), and mount the ideal panel as an **inline card** at the dispatch message via a `conversationEvents` `swarm-card` node keyed on `conversation.chat.node` | `client.test.mjs` + `swarmTab.test.tsx` + `scripts/verify-inline-chat.mjs`/`verify-interactions.mjs` (7/7 each) |
| 20 | Canvas "Open Workspace" opened the workspace panel invisibly **behind** the canvas | `embed.tsx` renders the canvas as a full-window fixed overlay (`zIndex 300`), which sat ABOVE the workspace panel (`zIndex 200`) | Raise the workspace slide-over to `zIndex 400` so it overlays the canvas **without** dismissing it (closing it returns to the still-open canvas) | `embed.test.tsx` "canvas agent panel 'Open Workspace' opens the workspace panel over the still-open canvas" |
...
| 23 | Clicking a resource pill in the canvas opened the menu in **every** place that resource appeared, and the menu got clipped/occluded | `ResourcePill` keyed its open state on the resource **id** (shared across agent panel / node / shelf), and used `position:absolute` inside a scroll container | Per-instance `menuKey` (context + id) + `position:fixed` anchored to the pill's viewport rect | `OrchestratorBoard.test.tsx` + manual browser check |
| 24 | Canvas showed no edges — orchestrator had no reports | Plan `ownerId`s were shorter than the recruited agent ids (`"html"` vs `"html-builder"`), so `deriveLines` matched nothing | `resolveAgentId()` (exact → normalized → prefix); used in `setPlan`/`deriveLines`/`state` | `host-e2e` "plan ownerIds resolve to agent ids for delegation/report edges" |
| 25 | `concurrencyLimit` was display-only — 20 agents spawned at once | `confirm` did `Promise.all(pending.map(spawn))` | `confirm` drains in **waves** of `concurrencyLimit` (spawn batch → await settle → next batch) | `host-e2e` "confirm drains subagents in waves of concurrencyLimit" |
| 26 | Every agent's artifacts landed in the shared pool | `deriveResources` emitted `zone: "shared"` for all artifacts | Emit `zone: ownerId` (**exclusive** by default; movable to shared) | `host-e2e` (resource zone === ownerId) |
| 27 | Test sessions/workspaces were inconsistent (per-round workspaces, generic names) | runner used a per-round workspace dir + generic session title | One workspace per **test type** (`dsh-agent-swarm-tests/<test>`), session named `swarm experiment <round>` | `scripts/_run-session-test.mjs` |
