...
| Client functional | `test/client.test.mjs` | The browser bundle (`lib/client.js`) registers the `conversation.view` "Swarm" slot and the `conversation.input.dock` live seat, and renders without a browser (vm sandbox + stub React). | `node --test` |
| Swarm-tab render | `dsh-agent-swarm-ui/src/components/__tests__/swarmTab.test.tsx` | Renders the client bundle with real React + jsdom: the `conversation.view` tab mounts `window.IdealSwarmUI.mount(container, {sessionId})`, and the `input.dock` seat mounts the card once the swarm is dispatched. | `pnpm test:run` (in the UI) |
...
| Live-chat browser check | `scripts/verify-inline-chat.mjs` | **Real GUI**: opens a completed swarm session at `127.0.0.1:3080` and asserts the ideal agent cards mount in the live dock while the Swarm tab still works. | `node scripts/verify-inline-chat.mjs` |
| Interaction browser check | `scripts/verify-interactions.mjs` | **Real GUI**: drives the live card — agent click opens the popup, canvas opens the board, percent toggle applies. | `node scripts/verify-interactions.mjs` |
...
| 19 | The swarm card only appeared after the whole job finished | It was mounted at `conversation.chat.turnTail` (turn **end**); a toy swarm is one ~76s turn, so the tail only renders after `summarize` | Mount at `conversation.input.dock` instead, gated on `firstSwarmTurn(snapshot) >= 0`, so the card appears **while subagents run** and live-updates | `client.test.mjs` `LiveSwarmDock` + `swarmTab.test.tsx` + `scripts/verify-interactions.mjs` |
